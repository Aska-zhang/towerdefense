...
monster3::monster3(moveway *startpoint,int tagg,int hp, playscene *gamee,QString picture)
...
void monster3::draw(QPainter *painter) const
{
    if (!alive)
        return;
...
//    painter->rotate(rotationSprite);
    ...
    QPixmap pix;
//    pix.load(":/res/Monsters1/pic5.png");
    pix.load(pic);
    pix.scaled(160,160);
